...
async def _handleHotAv_h5(background_tasks, q, cache, isSearch=False):
    ...
    if not isSearch:
        num = int(q)
        url = f"https://hugface.xiaodu1234.xyz/hot/{num}/?mode=async&function=get_block&block_id=list_videos_common_videos_list&sort_by=video_viewed&_={time_now}"
        cache_url = f"https://hugface.xiaodu1234.xyz/hot/{num}/"
        if num > 1477 or num < 0:
            return {"message": []}
    else:
        url = f"https://hugface.xiaodu1234.xyz/search/{q}/"
        cache_url = url
    ...
    async with httpx.AsyncClient() as client:
        ...
        urls_links = []
        for url in box:
            url = url.replace('https://jable.tv', 'https://hugface.xiaodu1234.xyz')
